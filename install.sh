...
# Configurer Grafana (HTTP sur port 3000)
GRAFANA_INI="/etc/grafana/grafana.ini"
[[ ! -f "${GRAFANA_INI}.orig" ]] && cp "${GRAFANA_INI}" "${GRAFANA_INI}.orig"

# Forcer HTTP (pas HTTPS par défaut pour éviter problème de cert)
sed -i 's/^;protocol = .*/protocol = http/'         "${GRAFANA_INI}" || true
sed -i 's/^protocol = .*/protocol = http/'           "${GRAFANA_INI}" || true
...
DS_RESPONSE=$(curl -sf \
    ...
    -X POST "http://localhost:${GRAFANA_PORT}/api/datasources" \
    -H "Content-Type: application/json" \
    -d "{
        \"name\": \"PostgreSQL\",
...
echo -e "  ${CYAN}Grafana       :${NC}  http://${IP}:${GRAFANA_PORT}"
echo -e "  ${CYAN}  login       :${NC}  admin / ${GRAFANA_ADMIN_PASS}"
echo -e "  ${CYAN}PostgreSQL    :${NC}  localhost:5432  db=${PG_DB}  user=${PG_USER}"
echo ""
echo -e "  Dashboard déployé : ${CYAN}http://${IP}:${GRAFANA_PORT}${NC} → '🍯 Honeypot Dashboard'"
echo ""
